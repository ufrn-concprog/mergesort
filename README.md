...
Especificamente para a versão concorrente da implementação do *merge sort*, deverão ser utilizadas *threads* **apenas** na etapa de ordenação propriamente dita, ou seja, a entrada e saída de dados devem ser sequenciais. Ademais, essa versão essencialmente consiste em alocar uma nova *thread* para tratar a partição do arranjo que está sendo ordenada.
